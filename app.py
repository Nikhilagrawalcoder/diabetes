...
# Application title
st.title("🩺 Diabetes Risk Prediction with Logistic Regression")
...
# Display feature importance using coefficients for Logistic Regression
...
plt.title("Feature Importance (Logistic Regression Coefficients)", fontsize=18, color="#2c3e50")
plt.bar(range(len(coefficients)), np.abs(coefficients[indices]), align="center", color="#2980b9", edgecolor="#2c3e50")
plt.xticks(range(len(coefficients)), features[indices], rotation=45, ha='right', fontsize=12)
